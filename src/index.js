...
module.exports = class ManboDebugger {
  /**
   * Main Client of ManboDebugger
   *
   * @param {Manbo.Client} client Discord Client
   * @param {ManboDebuggerOptions} options Manbo-debugger Options
   */
  constructor (client, { aliases = ['debug', 'debugger'], owners = null, prefix, secrets = [], noPerm, disableAttachmentExecution = false, globalVariable = {}, isOwner = undefined } = {}) {
    if (!(client instanceof Manbo.Client)) throw new Error('Invalid `client`. `client` parameter is required.')
    ...
    client.once('ready', () => {
      if (!this.owners) {
        console.warn('[Manbo-Debugger] Owners not given. Fetching from Application.')

        client.getOAuthApplication().then(data => {
          this.owners = data.team?.members?.map(member => member.user.id) || [data.owner.id] || []
          console.info(`[Manbo-Debugger] Fetched ${this.owners.length} owner(s): ${this.owners.length > 3 ? this.owners.slice(0, 3).join(', ') + ` and ${this.owners.length - 3} more owners` : this.owners.join(', ')}`)
        })
      }
    })
  ...
  /**
   * @param {Manbo.Message} message Message
